...
function renderCreatePage() {
    return `
        ...
        <!-- Publish Button -->
        <button onclick="publishDream()" class="w-full py-4 bg-gradient-to-r from-primary to-purple-800 rounded-full text-white text-lg font-bold shadow-[0_0_20px_rgba(200,128,255,0.4)] btn-hover">
            🚀 发布梦想
        </button>
    `;
}
...
function renderDetailPage(dreamId) {
    ...
    loadDreamComments(dreamId).then(comments => {
        const commentsContainer = document.getElementById('dream-comments-list');
        if (commentsContainer) {
            commentsContainer.innerHTML = comments.length > 0
                ? comments.map(comment => renderComment(comment)).join('')
                : '<p class="text-slate-500 text-sm text-center py-4">还没有评论，快来抢沙发！</p>';
        }
    });
    ...
    return `
        ...
        <div class="fixed bottom-0 left-0 right-0 bg-gradient-to-t from-[#1a0f23] via-[#1a0f23]/95 to-transparent pt-4 pb-8 px-5 z-40 max-w-md mx-auto">
            <div class="flex gap-3">
                ...
                <button onclick="submitComment(${dreamId})" class="flex items-center justify-center w-10 h-10 rounded-full bg-primary text-white hover:bg-primary/80 transition-colors shrink-0">
...
async function sponsorDream(dreamId) {
    ...
    try {
        ...
        const dream = getDreamById(dreamId);
        if (dream) {
            dream.energy = result.dreamEnergy;
            dream.supporters = result.dreamSupporters;
        }
        showNotification('赞助成功', `已为梦想赞助10能量！`, 'success');
        loadPageContent(currentPage, { dreamId });
    } catch (error) {
        showNotification('赞助失败', error.message, 'error');
    }
}
...
async function claimDailyReward() {
    try {
        ...
        localStorage.setItem('lastClaimDate', new Date().toISOString().split('T')[0]);
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (!success) {
        content.innerHTML = '<div class="flex flex-col justify-center items-center py-24"><p class="text-red-400 text-lg mb-2">⚠️ 数据加载失败</p><p class="text-slate-500 text-sm">请检查后端服务是否启动</p><button onclick="location.reload()" class="mt-4 px-4 py-2 bg-primary/20 text-primary rounded-lg text-sm">重试</button></div>';
        return;
    }

    // Initialize router (loads first page)
    initializeRouter();
...
function showNotification(title, message, type = 'info') {
    ...
    toast.innerHTML = `
        ...
        <button onclick="this.parentElement.remove()" class="text-slate-500 hover:text-white"><span class="material-symbols-outlined text-sm">close</span></button>
    `;
...
async function submitComment(dreamId) {
    const input = document.querySelector('#page-content input[type="text"]');
...
async function publishDream() {
    const title = document.querySelector('#page-content input[type="text"]')?.value;
    const desc = document.querySelector('#page-content textarea')?.value;
    if (!title || !title.trim()) {
        showNotification('请输入标题', '梦想需要一个名字', 'info');
        return;
    }
    try {
        await api.createDream({ title: title.trim(), description: desc?.trim() || '', category: '科幻未来', goal: 1000, impossibleIndex: 60 });
